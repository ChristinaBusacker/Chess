...
export class QueenChessPiece extends ChessPiece {
    public override type: EChessPiece = EChessPiece.Queen;
    ...
    public override getPossibleMoves(): IPossibleMove {
        ...
        if (possibleMoves.untilIsBlocked) {
            for (let i = 1; i < 8; i++) {
                if (parseInt(row) + i <= 8) {
                    possibleMoves.untilIsBlocked[0].push(
                        boardColumns[columnIdx] + (parseInt(row) + i)
                    );
                }
            ...
            for (let i = 1; i < 8; i++) {
                if (parseInt(row) + i <= 8) {
                    if (columnIdx + i <= 7) {
                        possibleMoves.untilIsBlocked[4].push(
                            boardColumns[columnIdx + i] + (parseInt(row) + i)
                        );
                    }

                    if (columnIdx - i >= 0) {
                        possibleMoves.untilIsBlocked[5].push(
                            boardColumns[columnIdx - i] + (parseInt(row) + i)
                        );
                    }
                }
                ...
                if (parseInt(row) - i >= 0) {
                    if (columnIdx + i <= 7) {
                        possibleMoves.untilIsBlocked[6]?.push(
                            boardColumns[columnIdx + i] + (parseInt(row) - i)
                        );
                    }

                    if (columnIdx - i >= 0) {
                        possibleMoves.untilIsBlocked[7]?.push(
                            boardColumns[columnIdx - i] + (parseInt(row) - i)
                        );
                    }
                }
